...
Список скриптов для работы с feature-flags:

| Скрипт            | Описание                                                                                                        |
| ----------------- | --------------------------------------------------------------------------------------------------------------- |
| `remove-features` | Автоматически удаляет фичи по названию флага. [Подробнее](/scripts/features/remove-features/remove-features.md) |
...
Работа с Feature-flags производится только через хелпер `toggleFeatures`, где в качестве аргумента передается объект со следующими полями:

-   `name` - название флага
-   `on` - стролочная функция, отрабатывающая после включения фичи
-   `off` - стролочная функция, отрабатывающая после выключения фичи
...
_Для автоматического удаления фичей используется скрипт [remove-features](/scripts/features/remove-features/remove-features.md)_
...
-   Настроен [Github Actions pipeline](.github/workflows/main.yml). Автоматические тесты при pull-request или push в master. Добавлены скриншотные тесты.
